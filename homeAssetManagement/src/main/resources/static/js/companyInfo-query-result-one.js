 ...
 */
	    ...
	    function companyInfoModify(id,event){
	    	...
	    	var tseOld = $("#tse"+id).text();
	    	var signatureCounterOld = $("#signatureCounter"+id).text();
	    	var seriesNumberOld = $("#seriesNumber"+id).text();
	    	var ustOld = $("#ust"+id).text();
	    	var telephoneNumberOld = $("#telephoneNumber"+id).text();
	    	var webSiteOld = $("#webSite"+id).attr("data-text");
	    	    webSiteOld = (webSiteOld == '0')?'':webSiteOld;
	    	...
	    	$("#tse"+id).html('<input type="text" id="tseInput'+id+'" value="'+tseOld+'" style="width:100%"/>');
	    	$("#signatureCounter"+id).html('<input type="text" id="signatureCounterInput'+id+'" value="'+signatureCounterOld+'" style="width:100%"/>');
	    	$("#seriesNumber"+id).html('<input type="text" id="seriesNumberInput'+id+'" value="'+seriesNumberOld+'" style="width:100%"/>');
	    	$("#ust"+id).html('<input type="text" id="ustInput'+id+'" value="'+ustOld+'"  style="width:100%"/>');
	    	...
	    	// 为 取消修改 做准备
	    	$("#companyInfoModifyCancel"+id).attr("onclick","companyInfoModifyCancel('"+id+"',event,'"+nameOld+"','"+tseOld+"','"+signatureCounterOld+"','"
	    	  +seriesNumberOld+"','"+ustOld+"','"+telephoneNumberOld+"','"+webSiteOld+"','"+openTimeOld+"','"+mailingAddressIdOld+"','"+creatDateOld+"','"+lastModifyDateOld+"')");
	    	
	    }
	    
	    
	    // 取消修改
	    function companyInfoModifyCancel(id,event,nameOld,tseOld,signatureCounterOld,seriesNumberOld,ustOld,telephoneNumberOld,webSiteOld,openTimeOld,mailingAddressIdOld,
	    	...
	    	$("#tse"+id).text(tseOld);
	    	$("#signatureCounter"+id).text(signatureCounterOld);
	    	$("#seriesNumber"+id).text(seriesNumberOld);
	    	$("#ust"+id).text(ustOld);
	    ...
	    function companyInfoModifyConfirm(id,event){
	    	...
	    	var tse = $("#tseInput"+id).val();
	    	var signatureCounter = $("#signatureCounterInput"+id).val();
	    	var seriesNumber = $("#seriesNumberInput"+id).val();
	    	var ust = $("#ustInput"+id).val();
	    	...
	    	var companyInfoJson = {"id":id,"name":name,"tse":tse,"signatureCounter":signatureCounter,
	    	                   "seriesNumber":seriesNumber,"ust":ust,"telephoneNumber":telephoneNumber,
   			                   "webSite":webSite,"openTime":openTime,"mailingAddressId":mailingAddressId,"creatDate":creatDate};
	    ...
	    function afterCompanyInfoModifyConfirm(result){
	    	...
	    	$("#tse"+id).text(result.tse);
	    	$("#signatureCounter"+id).text(result.signatureCounter);
	    	$("#seriesNumber"+id).text(result.seriesNumber);
	    	$("#ust"+id).text(result.ust);
